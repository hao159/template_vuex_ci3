...
class Mongo_db
{
     ...
     *  get(), get_Where(), getOne(), getOneWhere(), count().
     *
     *  @var boolean
     */
     ...
     *
     *  @uses     $this->mongo_db->get_Where('users', ['user.name' => 'Alexter']);
     *
     *  @param    string  $collection  [Collection name]
     ...
     *  @return   array
     */
    public function get_Where(string $collection = '', array $where = [], array $options = []): array
    {
    ...
    {
        $tmp = $this->_get($collection, ['limit' => 1] + $options, false, __METHOD__);
        if ($tmp) {
            return $tmp[0];
        }
        return [];
    ...
    {
        if ($collection == '') {
            $this->error('Specify collection name to which query will be applied');
        }
        ...
        try
        {
            ...
            if ( $this->wheres) {
                $arrCommand["query"] = (object) $this->wheres;
            }

            $command = new Command($arrCommand);
            $Result = $this->db->executeCommand($this->db_name, $command);
            $this->reset_query();            
            return $Result->toArray()[0]->n;
        } catch (Exception $e) {
            $this->error("Failed to complete query to count data: {$e->getMessage()}");
        }
    ...
    public function add_index($collection = "", $keys = array(), $options = array())
    {
        ...
        if ($this->_command($command, __METHOD__)) {
            $this->reset_query();
            return ($this);
        } else {
    ...
    /**
     *  get distinct data by hao.nguyen :))
     *  
     *  @param   string  $collection  <collectionName>
     *  @param   string   $field   <field>
     *  @return  $data
     */
    public function get_distinct(string $collection = '', string $field=''): array
    {
        if ($collection == '') {
            $this->error('Specify collection name to which query will be applied');
        }
        if ($field == '') {
            $this->error('Specify field name to which query will be applied');
        }
        ...
        try
        {
            ...
            $Result = $this->db->executeCommand($this->db_name, $cmd);
            $this->reset_query();
            return $Result->toArray()[0]->values;
        } catch (Exception $e) {
            $this->error("Failed to complete query to count data: {$e->getMessage()}");
        }
